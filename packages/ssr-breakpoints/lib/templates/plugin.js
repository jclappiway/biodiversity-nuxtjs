...
export default async function ({req, store,isServer,isClient, inject, router}) {
  ...
  router.afterEach(function (to, from) {
    if(!cssBreakpoint) getCSSBreakpoint()
  ...
  function isMobile () {
    ...
    if (!cssBreakpoint && isServer && !cookieBreakpoint)
  ...
  function isTablet () {
    ...
    if(isClient)
      result = (getCSSBreakpoint() === 'isTable')

    if(result) setCookieItem('breakpoint','isTable',Infinity,'/')
  ...
  function isTouch () {
    ...
    if(isClient)
      result = (getCSSBreakpoint() === 'isTable' || getCSSBreakpoint() === 'isMobile')
  ...
  function isDesktopWideScreen () {
    ...
    if(!cssBreakpoint && isServer && cookieBreakpoint)
      result =  (cookieBreakpoint ==='isDesktopWideScreen')

    if(isClient)
      result = (getCSSBreakpoint() === 'isDesktopWideScreen')

    if(result) setCookieItem('breakpoint','isDesktopWideScreen',Infinity,'/')
  ...
  function isDesktopHighDefinition () {
    ...
    if(!cssBreakpoint && isServer && cookieBreakpoint)
      result =  (cookieBreakpoint ==='isDesktopHighDefinition')

    if(isClient)
      result = (getCSSBreakpoint() === 'isDesktopHighDefinition')

    if(result) setCookieItem('breakpoint','isDesktopHighDefinition',Infinity,'/')
